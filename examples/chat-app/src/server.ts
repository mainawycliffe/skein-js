// Serve this example's `langgraph.json` (the `research` graph) over HTTP with @skein-js/express —
// the programmatic equivalent of what `skein dev` boots in process. Run it with `tsx src/server.ts`
// (PORT overridable), or import `startServer` in a test to drive it with the LangGraph SDK.
